...
<span class="screen-path">Accessibility → Activity</span> (administrators only) shows who changed what, and when: fixes, alerts, reports, the license, settings and dismissed issues.

Filter by:

- **Show**: All activity, Fix tracking, Alerts, Reports, License, Settings or Dismissed issues.
- **By**: a person, or **Automatic** (Lumtera, scheduled tasks, WP-CLI).
- **From** and **To** dates.
...
Under **How long to keep activity**, set **Keep entries for** a number of days. The default is **365**, and 0 keeps everything. Older entries are removed once a day.
...
<ol class="step-list">
  <li>Enter the <strong>Address</strong>. It must use <code>https://</code> and a public host.</li>
...
You can add up to 10 webhooks. Turn off **Enabled** to pause one without losing its settings. Each webhook shows its last delivery and HTTP status.
...
| Group | Events |
| --- | --- |
| Fix tracking | `task.created`, `task.assigned`, `task.status_changed`, `task.note_added`, `task.verified`, `task.reopened`, `task.dismissed` |
| Alerts | `alert.new_errors`, `page.new_errors` |
| Reports | `report.created`, `report.deleted` |
| License | `license.changed` |
| Settings | `settings.changed` |
| Dismissed issues | `issue.dismissed`, `issue.restored` |
...
```json
{
  ...
  "time": "2026-09-25T09:30:00+00:00",
...
`actor.type` is `user` or `system`. For system events, `actor.name` is "WP-CLI", "Scheduled task" or "Lumtera". Payloads never contain license keys, secrets or setting values.
